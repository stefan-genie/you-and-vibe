...
export interface ChatIdeContent {
  mode: "generate" | "fix";
  intro?: string;
  introBeforeUrl?: string;
  introAfterUrl?: string;
  hints: string[];
  brokenCode?: string;
  errorLog?: string;
  serviceUrl?: string;
...
export const tasks: Task[] = [
  ...
  { id: "t3", type: "realChat", difficulty: "medium", title: "Живой разговор", unlocksAfter: "t2", content: { hasJudgeStep: true, intro: "Желе-кубик закончил рабочий день на архивной станции, но голова раскалывается уже третий час. Нужно написать начальнику, что сегодня придётся уйти пораньше. Первая мысль — просто написать «у меня болит голова» — но вдруг это прозвучит как отмазка? Желе решает сперва посоветоваться с ИИ, прежде чем составлять сообщение. Помоги ему — спроси у своего ИИ-помощника, как лучше сформулировать, чтобы это звучало и честно, и профессионально." } },
  ...
  { id: "t5", type: "chatIde", difficulty: "hard", title: "IDE-ассистент: автодополнение", unlocksAfter: "t4", content: { mode: "generate", introBeforeUrl: "Рабочий день почти закончен, но желе не может уйти, пока не убедится, что его сервис отвечает. В блокноте записан адрес: ", serviceUrl: "http://jelly-service:8080/jelly/status", introAfterUrl: " — желе хочет попросить ИИ написать скрипт, который дёрнет этот адрес и покажет, всё ли в порядке. Опиши эту задачу своими словами в чате: куда стучаться, что именно проверить, как понять, что всё хорошо.", hints: ["Подумай, что именно ты просишь проверить — статус-код, тело ответа, время?", "Скажи ИИ, в каком виде ты хочешь увидеть результат.", "Если скрипт падает с ошибкой соединения — возможно, стоит добавить timeout."] } },
  { id: "t6", type: "chatIde", difficulty: "hard", title: "Рефакторинг кода с ИИ", unlocksAfter: "t5", content: { mode: "fix", intro: "Скрипт, который желе написал вчера вечером, сегодня утром перестал работать. Вот что желе видит в логе, когда пытается его запустить. Разберись, в чём может быть проблема, и попроси своего ИИ-помощника помочь её найти и исправить — лог можно вставить в промпт, если тебе это поможет объяснить ситуацию.", hints: ["Сравни URL в коде с тем адресом, который желе записал в блокноте в прошлом задании.", "Посмотри на каждый символ в пути — иногда одна лишняя буква меняет всё.", "Опиши ИИ и код, и лог — так ему будет проще найти причину."], brokenCode: "import requests\nr = requests.get(\"http://jelly-service:8080/jelly/statuss\")\nprint(f\"RESULT_STATUS:{r.status_code}\")", errorLog: "RESULT_STATUS:404" } },
];
